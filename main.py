...
with open("bins.txt", "r") as f:
    bins = f.readlines()
    bins = [bin.strip() for bin in bins]

def bin_generator():
    with open("bins.txt", "r") as f:
        bins = [bin.strip() for bin in f.readlines()]
    num_bins = len(bins)
    i = 0
    while True:
        bin = bins[i]
        i = (i + 1) % num_bins  # Loop back to the first bin when the last bin is reached
        yield bin

bin_iter = bin_generator()

fullBin = next(bin_iter)
